...
public class AppManagerContact implements Runnable {
    ...
    public int splitAndSendTask(File InputFile, String Task_key, String bucket,
                                String LocalQueue) throws FileNotFoundException {
        int counter = 0;
        try {
            BufferedReader reader = new BufferedReader(new FileReader(InputFile));
            String line = reader.readLine();
    ...
    public void newTaskFromApp(Message msg) throws IOException {
        if (!terminate) {
            ...
            OCRfile(ourFile,
                    bucket); // creating an OCR file with the number of urls in the input message & sending it to s3
    ...
    public void terminateWorkers() {
        for (EC2 worker : DB.getWorkersList()) {
            worker.terminate();
        }
    }
